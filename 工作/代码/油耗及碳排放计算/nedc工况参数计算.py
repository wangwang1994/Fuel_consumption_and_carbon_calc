...
# 在这里把工况文件路径粘进去
data = pd.read_excel('/Users/xuchangmao/Desktop/工作/油耗行为分析/标准工况/标准模版/nedc工况.xlsx')
...
acc_info_pd = pd.DataFrame({'整体': acc_info,
                            '市区': urban_acc_info,
                            '市郊': sub_acc_info}, index=['加速段平均加速度', '减速段平均减速度', '最大正向加速度', '最大减速度'])
...
region_info_pd = pd.DataFrame({'市区': urban_region_info,
                               '市郊': sub_region_info}, index=['加速占比', '减速占比', '巡航占比', '怠速占比'])

rpa_list = [calc_region_rpa('urban', data), calc_region_rpa('sub', data)]
distance_list = [calc_region_distance('urban', data), calc_region_distance('sub', data)]
average_speed_list = [urban_speed_list[1], sub_speed_list[1]]
rpa_info_pd = pd.DataFrame({'平均车速': average_speed_list,
                            'rpa': rpa_list,
                            '里程': distance_list}, index=['市区', '市郊'])
